var store = [{
        ...
        "excerpt":"그 동안 자바스크립트 공부를 하면서 명확하게 잡히지 않았던 것들만 골라 경험담과 함께 시리즈로 정리하려한다. 카페트 밑의 먼지 마냥 급하진 않았지만 거슬리는 기본적 개념, 근본적인 동작에 대한 갈망을 이번 시리즈로 해결하고자 한다.   SCOPE   let, const 같은 키워드는 ES의 패러다임 바꾼다. 좋다고 하니까, 일단 let을 쓰라는 가이드만 따르다보니 let을 쓰지 않음으로서 생기는 오류를 접할 기회가 없었다. 결국 [지역, 전역] 그리고 [var, let, const]의 차이와 구체적인 동작을 명확히 모른채 약간 혼란스러운 상태에 있었다. JSLint 를 쓰면 문제를 바로 지적해주니 당장 문제는 회피할 수 있지만 도대체 무슨일이 벌어지는지, 어디에서 어디로 프레임이 옮겨 갔는지 알아야 한다고 생각한다.   (1) ~ (4) 이슈로 쪼개어서 앞으로 설명ISSUE (1) 지역변수와 전역변수   let을 만나고 내가 헛갈린건 대다수 언어가 ‘블록단위 스코프’를 쓰기 때문이다. 자바는 전역을 쓰고 싶다면 그냥 static 을 붙인다. 그덕에 ‘JS에서는 var, const가 static 인가?’라고 착각을 했다. 아니다. 지역과 전역은 let 같은 선언 키워드의 문제가 아니다.   ISSUE (2) 함수 스코프의 부연설명   이 자바스크립트의 맹점을 기계적으로 회피하는게 아니라 실제로 이해하려면 결국 글로벌 객체와 실행문맥을 선언 키워드와 연결해서 이해해야한다.   — 자바스크립트 함수 스코프가 기본이다.   — 명시되지는 않았지만 작성된 코드는 브라우저의 window객체를 기준으로 실행된다.    — 즉 브라우저측에서 우리가 다루는 전역변수의 실체는 window 객체의 프로퍼티다. (엄밀히 따지면 같은 전역변수라도 var와 let은 다르다. let은 window프로퍼티가 아니다. 아래에서 정리한다.)   전역객체와 변수의 관계— 첫 예제의 test 외부에서 선언된건 모두 전역변수다. 다른 실행컨텍스트 즉 다른 함수에서 전역변수를 선언하는 방법은 (3)에서 설명    —따라서 예제의 test 함수 안에 선언된것들은 무슨 키워드를 쓰든 지역변수다. 즉 키워드가 아니라 어떤 함수스코프에서 선언이 되었느냐가 변수의 호출범위를 좌지우지한다.   — 그렇게 의미가 없다면 선언 키워드 간 차이는?   함수 스코프를 따르고 재선언 및 변수변경이 가능한 &lt; var &gt;   블록 스코프를 사용하고 재선언이 불가능한 공통점을 가진 &lt; let &gt;, &lt; const &gt;   글로벌객체 (또는 실행함수)에 프로퍼티를 형성하는 &lt; var &gt;  글로벌객체 (또는 실행함수)에 프로퍼티를 형성하지 않는 &lt; let &gt;, &lt; const &gt;  &lt; let &gt;과 &lt; const &gt;의 차이점은 값 변경가능여부다.   var는 글로벌 객체에 붙었다. 그럼 let과 const는 어디에 있다가 호출되는걸까let은 어디에 숨어 있는지 알기 위해. 익명함수를 실행했다. 함수는 실행될때마다 저 자신의 실행문맥을 가진다. (실행문맥은 함수의 argument, scope 등을 가지고 있다) 여기선 함수를 ( ) 으로 래핑함으로서 윈도우와 동떨어진 독립된 스코프가 되었다. 내가 콘솔에서 선언한 testLet은 글로벌객체엔 없지만 아니지만 실행문맥의 Script Scope 에서 찾을 수 있기에 Ref 에러를 발생시키지 않았다.ISSUE (3) 어떻게 전역을 선언하나?   전역(글로벌객체)에 접근하는것 그게 전역변수를 만드는 방법이다. 그리고 이글은 3번 Line에 대한 긴 설명이었다.첫 예제의 ‘noVar’ 처럼 그냥 키워드 없이 쓰는게 JAVA의 static 에 해당한다는걸 알수 있다. 특별히 정하지 않거나 var를 사용하면 변수가 죄다 전역객체에 붙어버린다. 혼자만 쓰는것도 아닌데 글로벌 객체에 덕지덕지 붙는건 문제를 일으킬것 같다. let으로 블럭 스코프를 기준으로 변수를 관리하는 것이 의미가 있다는걸 알 수 있다.   var는 실행중인 함수의 프로퍼티에 접근하는 키워드다. 이게 이번 글의 핵심이다. 익명함수 안에서 실행된 var는 수행이 끝남과 동시에 컨텍스트가 사라져 다시 호출할 수 없다. 로컬변수는 이렇게 사라지고 그걸 사라지지 않기 위한 패턴이 클로져다.ISSUE (4) 함수의 내용은 처음부터 실행 되지는 않는다.  let, var, function, class, {} 같은 선언(statement)은 코드가 읽힐때 호이스팅된다. 이때 호이스팅은 선언과 실행의 순서가 거꾸로라도 Reference 에러가 나지 않는 차이가 있는거지 내용물을 실행하는건 아니다. 리터럴 형식으로 정의된 함수도 마찬가지다. var 에 undefine이 먼저 정의할뿐이지 할당은 순차적인 작업에서 시행된다.   — 그렇기 때문에 예제에서 test() 하기 전 noVar의 호출은 에러를 일으키지만 호출후에는 전역변수가 되어서 스코프에 잡혀 호출에도 에러가 발생하지 않는다.   — 하지만 바로 실행되지 않는다고해서 실시간으로 호출되는 형태에 따라 Scope가 정해진다고 생각하면 안된다. 스코프는 함수가 사용할 준비가 될때 (호이스팅 될때 정해진다.)   foo() 에서 콜 스택은 global, foo, bar 순으로 쌓이겠지만 그렇다고 호출해준 함수의 스코프를 순차적으로 참고하지 않는다. bar() 에서 콜 스택은 global, bar 순으로 쌓이고 전역변수가 호출되는게 자연스러워 보이지만 원래 스코프는 global 밖에 없었다.스코프는 바뀌지 않는다. 어떻게 호출하든 함수의 위치만이 영향을 미친다는 거다. 처음 정해진것을 따라간다는 이야기다.   로컬에 변수가 없으므로 글로벌을 참조한다. 여기서 함수 bar 는 전역객체 스코프만 가지고 있다. 콜스택은 그저 프로그램 제어순서를 보여줄뿐이다.bar 함수는 foo(); 를 통해 사용할 준비가 된다. 전과 다르게 foo의 컨텍스트에서 호이스팅되는 bar이므로 로컬변수가 출력된다.  ","categories": [],
        "tags": [],
        "url": "/%EC%9E%90%EB%B0%94%EC%8A%A4%ED%81%AC%EB%A6%BD%ED%8A%B8-%EC%98%A4%ED%95%B4%ED%92%80%EA%B8%B0-1-%ED%8E%B8-SCOPE/",
        "teaser": "/assets/images/HibikeQauntumSymbol_500x300_version_waifu.jpeg"
      },{
        "title": "Graph, Tree, Hash Tables, Binary Tree (B-Tree)",
        "excerpt":"자료구조 정리 2편   Binary Tree### Tree      1편에서 알아본 자료구조는 각자 특징이 있었지만 모두 선형적구조를 가지고 있었다. 2편은 그렇지 않은 자료구조의 모임이며 Tree는 그중 Root를 가지고 하위노드를 거느린다는 특징을 가진다.   Tree를 규정하는 가장 중요한 원칙 ‘각 노드가 하나의 부모만을 가진다.’   하위노드를 몇 개를 가지는지, 하위노드의 갯수가 불완전해도 되는지, 깊이의 불균등, 데이터를 어떤 순으로 담을지에 따라 여러종류로 나눈다.   위아래가 있는 위계적 구조는 관료제도를 떠오르게 하는데 인간의 인지구조에 친화적인 구조이다. DOM 도 이러한 구조로 구성되어 있다.   좀더 복잡해진 구조인 만큼 노드들이 위치에 따라 호칭을 가진다. 객체지향프로그래밍이나 여러 OS의 파일시스템도 Tree의 모양새이기 때문에 익숙할 단어들도 있다.  Root, Child, Parent   Brother,Sibling Node — 어떤 노드의 같은 레벨에 있는 노드   Leaf Node — 자식 노드가 존재하지 않는 끝단 노드  Branch Node — 자식 노드가 있지만 Root가 아닌 중간노드   Edge — 노드간의 관계를 나타냄        Tree를 관리하기 위해서 Add, Search, Delete, Change 등의 메서드가 필요 B-Tree Pseudo### B-Tree       이진 트리는 각각의 노드가 최대 두 개의 자식 노드를 가지는 트리 자료구조를 말함. 더 많은 자식을 가지는 트리를 B트리라함.        B-Tree는 시간복잡도면에선 뛰어남. 데이터베이스에서 많이 사용. Graph### Graph       Graph는 Tree와 비슷하지만 Tree에 있는 몇 가지 제약이 없는 사실상 Tree의 상위호환격 자료구조이다. 몇개의 자식노드(인접노드)를 가지는지 제약도 없고 root도 없기 때문이다.   Directed Graph, Undirected Graph로 나뉜다. (Tree는 Directed)   트리구조가 질서정연한 관료제도를 떠올리게 한다면 Graph는 현실에 있는 복합적인 구조를 떠올리게 한다. 다차원적으로 얽힌 인간관계, 골목길과 고속도로, 고가도로 등이 복합적으로 존재하는 도로망 이러한 현실은 Graph로 구현하기에 적합하다.        다른 노드에서 오는 엣지의 개수를 In-degree 다른 노드로 가는 엣지의 개수를 Out-degree라 한다. (방향이 있는 Graph에서 표현되는 방식) 트리는 이 degree의 갯수가 트리의 규칙에 예속되어 있었지만 Graph는 제약이 없다. Graph PseudoSeparate chaining 방식 해쉬테이블### Hash table       배열은 빠른 탐색속도를 보장한다. 하지만 배열에 직접 데이터를 담으면 비용이 커진다. Data가 동영상이라면 더욱더 그러하다. 해시테이블은 이런 문제를 해결하기 위한 자료구조이다.   데이터를 해시함수에 집어넣어서 배열의 인덱스(버켓)로 활용할 수 있는 값을 얻는다. 그리고 그것을 인덱스의 크기로 나눠 나머지로 인덱스를 결정한다. 특정 인덱스에 몰리면 탐색비용이 증가하기 때문에 모든 인덱스에 골고루 분포되도록 해시함수를 만드는게 중요하다.   해수함수가 만들 수 있는 결과값 종류보다 인덱스 사이즈가 작을수록 더 많은 중복이 생기는데 그 때 2차탐색을 처리하는 방식은   인덱스에 Linked List를 할당하는 Separate chaining  아직 사용되지 않은 Hash에 중복되는 값을 넣어버리는 Open addressing HashTable Pseudo  ","categories": [],
        "tags": [],
        "url": "/graph-tree-hash-tables-binary-tree-b-tree/",
        "teaser": "/assets/images/HibikeQauntumSymbol_500x300_version_waifu.jpeg"
      },{
        "title": "Stack, Queue, Linked List",
        "excerpt":"자료구조 정리 1편   Stack### Stack      LIFO(Last In First Out) 자료구조이다.   가장 최근에 일어난 일을 신경쓰고 빨리 처리하는 점에서 인간의 의식과도 닮아있다. 이때 처리되지 못하고 못한 오래된 ‘A’같은 일은 의식 바닥에 깔려있게 된다.        그 대신 ‘D’나 ‘C’같은 같은 최근 작업은 반응속도를 보장한다는 장점이 있다. 이런 자료구조를 다루기 위해서는 일을 넣는 push, 일을 빼는 pop 그리고 전체적으로 쌓여있는 양을 측정할 peek 같은 메서드가 필요하다. Stack PseudoQueue### Queue       FIFO (First In First Out)의 원칙으로 처리하는 자료구조이다. 은행의 대기열과 비슷하다. 대기열이 짧으면 바로 일을 처리하는 기적도 있을 수 있지만 대개는 줄을 서야한다.   그런 단점이 있지만 모든 작업이 공평하게 처리된다는 점에서 극단적인 대기시간을 가지는 문제는 없다는 장점을 가진다.        이런 Queue를 처리하기 위해선 가장 오래기다린 일, 즉 일들 중 가장 먼저 온 일을 처리하는 Dequeue. 새로 운 일을 대기열에 추가하는 enqueue. 대기열을 측정하는 Qlength가 필요하다. Queue PseudoLinked List### Linked List       Linked List는 Array같은 단일구조체와 대비되는 자료구조이다.   Data, Next Adress로 구성되고 next가 다음 데이터를 가르킨다. 각 정보가 분리되어 있지만 정보를 찾아낼 연락망은 다 갖추어진 구조이다.   배열이 순차적인 페이지번호가 쓰여진 질서정연한 인명기록부라면 Linked List는 단일인명정보다. 대신 일련된 페이지 번호대신 다음 인명정보가 어디있는지 기록되어있다.   새로운 정보를 기입할 일이 생기면 Array는 전체 페이지번호를 하나하나 변경해야하는 반면 Linked List는 종이 한 장에 있는 Next Adress를 새 Linked List로 고치는 정도로 변경이 끝난다. 그래서 출입력에는 강점을 가진다.   대신 탐색에는 시간이 오래걸린다. 원하는 사람을 찾기 위해서 처음부터 하나하나를 순서대로 다 들춰봐야하기 때문이다. Liked list Pseudo  ","categories": [],
        "tags": [],
        "url": "/stack-queue-linked-list/",
        "teaser": "/assets/images/HibikeQauntumSymbol_500x300_version_waifu.jpeg"
      },{
        "title": "자바스크립트 상속",
        "excerpt":"  ES5 스타일   ES5 style inheritance자바스크립트가 원래 OOP가 아닌만큼 상속을 구현하는게 상당히 상스러운 모습이다.      부모의 생성자함수 call(this, name)로 실행   Object.create() 카피   constructor 교체 위 3가지가 핵심 포인트   ES6 스타일   ES6 style inheritance‘class’로 상속을 구현한 코드다.      ‘extends’ 때문에 상속을 표현하는게 짧아졌다.   생성자를 자동으로 받아서 실행한다. (주석부분)        ‘super’ 키워드로 부모에 접근할 수 있다. (아래에서 소개) super exampleSuper의 사용예제다.       원래 7300일로 나와야할 papa의 dayOflived가 14600이 된것을 볼 수 있다. 자녀의 위치에서 age 변수를 2배로 바꿨다. 별로 좋은 예제는 아닌것 같지만 이상한 만큼. 이상한 동작의 원인이 될 수는 있다.   super.ping()을 통해서 부모의 함수를 호출한 모습을 볼 수 있다. 저 위치에서 super 없이는 부모객체에 접근할 수 없다. pingpong위치에선 this가 child를 보고 있다. super를 통해 부모에 접근하는 것이다.   super()를 constructor에 넣어주면 부모의 생성자를 그대로 실행한다는것을 명시할 수 있다.   ","categories": [],
        "tags": [],
        "url": "/%EC%9E%90%EB%B0%94%EC%8A%A4%ED%81%AC%EB%A6%BD%ED%8A%B8-%EC%83%81%EC%86%8D/",
        "teaser": "/assets/images/HibikeQauntumSymbol_500x300_version_waifu.jpeg"
      },{
        "title": "N-Queens 문제해결하기",
        "excerpt":"페어프로그래밍으로 하는 문제중에 가장 재미있는 시간이었다. 도전적이기도 하고 사실상 답이 정해져있는 다른 방법에 비해 자료구조를 동반하는 알고리즘은 방법에 제한이 없다보니 창의적인 방법을 시도하는 느낌이 드는게 굉장히 즐거웠다. 익숙하지 않은 남이 써놓은 구조체(코드)를 제한된 시간내에 빨리 파악하고 누군가 불친절하게 써놓은 코드를 거꾸로 해석해가는 것도 즐거운 작업이었다.   N-Queens 의 목표는N * N 사이즈의 보드에 rook 와 queen을 놓는 경우의 수와 놓은 모양의 배열을 만드는 것이다. 내가 푼 방식은 비효율적인것 같긴하지만 OOP를 도던적인 문제를 해결하는데 재미있게 사용한 첫 경험을 제공해주었다.   사용한 자료구조처음엔 절대로 사용하지 않을 자식은 생성하지 않는 구조를 만들다가 보드의 사이즈가 8을 넘기자 메모리 크래시를 경험할 수 있었다. 트리를 리프까지 채우기 위해 만든 메서드가 재귀인 문제의 원인이었다. Look든 Queen 이든 세로(컬럼)은 딱 한개의 말만 존재할 수 있다는 점을 이용해서 문제를 해결했다. 위의 빨간색 X 표시는 사용하지 말아야할 컬럼의 번호를 차일드에게 넘겨주어서 차일드가 재귀함수를 스마트하게 호출하게 한 덕에 만들지 절약한 메모리 공간이라고 볼 수 있다.   리프가 들고 있는 path는 Look의 배치도가 된다. 여기서 리프.path === N사이즈의 보드에서 모든 LOOK 경우의 수라고 보면 된다.   이제 만들어진 리프를 꺼내서 이번 과제가 제시한 목표를 클리어하면 된다. 아무 리프나 꺼내서 배열에 0,1 로 표시를 하면 된다. 이건 자료구조에 비하면 간단하다. 특히나 LODASH에서 제공하는 board Model은 배열에 값을 쉽게 반영하게 해준다. 원래 View, Model 구조를 알려주기 위해서 서비스로 넣은거라 생각했는데 생각보다 더 중요한 구조체였다.   Queens의 경우는 Look보다는 어려웠지만 더 빨리 풀었다. 처음엔 만들어진 path를 수동으로 파싱해서 대각선처리를 하려고 했는데 자신보다 높은 레벨의 대각선을 체크하는 문제에 직면하자 if 문 한 두개로 처리할 규모가 아니게 되었다. 그래서 일단 LOOK의 리프를 2중 배열에 집어넣고 대각선 체크함수를 활용하는 방법으로 우회하게 되었다.   개선할점      대각선 등 배열에서 충돌을 체크하는 함수가 약간 하드코딩된 느낌이 있다. 그래봤자 IF 두개 정도에 배열 사이즈에 영향을 전혀 받지 않을만큼 견고했지만, 배열의 성질을 활용하는 흥미로운 방법을 적용하면 더 흥미로운 코드를 만들 수 있을 것같다.   사이즈 1부터 사이즈 9까지 queens의 모든 가능성을 체크하는데 까지 1초가 걸렸다. (내 맥북이 13년형 인걸 감안하면 감탄스럽지만) 사실 OOP 대신 Queens나 Look가 보드 속에서 나타내는 성질을 활용하면 훨씬더 빠르게 문제를 처리할 수있다. (여기 OOP를 쓴건 닭잡는데 소잡는 칼을 쓴것같다)   ","categories": [],
        "tags": [],
        "url": "/n-queens/",
        "teaser": "/assets/images/HibikeQauntumSymbol_500x300_version_waifu.jpeg"
      },{
        "title": "React — props, state, life-cycle",
        "excerpt":"리액트를 배울때 가장 많이 언급되는 개념 그중 props, state, life-cycle을 정리하는글이다.   리액트의 존재이유가 뭘까?      동적인 View 구현   그럼에도 코드를 줄인다. 이런 목표를 중심으로 라는 관점에서 위 세가지 키워드를 살펴 보고자 한다. 그전에 리액트의 가장 큰 단위인 컴포넌트를 짚고 넘어가자.   리액트 할아버지 모임1. 먼저 import ::   React 구조체 ‘react’ 와 리액트 라이브러리가 몰래 뒤에서 관리하는 가상의 DOM인 ‘react-dom’ 을 가지고 왔다.     class App 상속  App 은 convention 하게 리액트의 최상위 컴포넌트에 할당하는 이름이다. 이 안에 컴포넌트를 계속 nested 하게 구현하여 우리가 원하는 복잡하지만 간단한 코드를 만들수 있게 된다. 컴포넌트는? 간단히 생각하면 사용자가 만드는 HTML 태그이자 리액트 프로젝트를 쌓아가는 블럭이다.   render 에선 다음은 우리눈에 보이게될 View를 설명하는 곳이다. 따라서 이곳 밖에선 이 View를 결정할 외적인 넣을 수 있다는 말이다. render 와 return 사이에는 렌더전 로직을 작성할 수 있다. 직접적으로 DOM은 아니지만 라이프사이클을 통해 매번 호출되어 로직을 작성할 수 있다. return 안의 문법은 JSX를 따르게 되는데 바닐라 리액트로 처리할때보다 코드량이 줄어드는 장점이 있다. 대신 Bable이 컴파일 해주는 단계가 추가된다. 디버깅할때 살짝 까다로워질 수 있다.   마지막으로 ReactDOM에 render 명령을 내려준 모습을 볼 수 있다.  블럭만을 대상으로 했지만 실제코드에선 여러가지 블럭이 App에 딸려있으므로 프로젝트가 통째로 렌더링 되는 모습을 볼 수 있다.     Props       Props는 컴포넌트를 사용할때 할당시켜주는 Data다. 코드내에서 수정하거나 임의로 추가하는것이 금지되어 있다. 부모 컴포넌트로 통신할 필요가 있다면 State를 위로 올려주는 방법을 사용한다.   구체적인 컴포넌트의 구조와 props의 운명에서 title과 list라는 이름의 props를 내려줬다. 에선 그 이름을 잘 활용하고 있는 모습을 보여주고 있다. 아까전에 render와 return 사이에서 여러로직을 처리한다고 했는데 이 코드는 list를 map으로 돌려 중복되는 을 한번에 찍어내기위한 준비를 했다. mappingLists를 return 에서 {mappingLists}의 형태로 사용하고 있는 모습이다. 대신 Item에는 key값이 필요하다는 경고를 해준다. 그래서 key라는 props를 일부러 지정해주었다.   State   State는 컴포넌트 단위로 가지는 자료창고다. 자료창고의 자료를 꺼내서 Props로 내려줘도 좋고 이후에 나올 라이프 사이클에서 만들 수 있는 로직을 state에 반영한다거나 여러가지 동작이 가능하다. 다만 this.state로 직접 접근하면 안되고 setstate 로직을 사용하여야 한다.   위는 각 done 이라는 state의 키값이 클릭에 의해 토글되는 로직을 구현했다. 일반 자바스크립트라면 동작하지 않을 내용이지만 render는 라이프사이클 또는 setstate가 호출될때마다 동작된다. 매 렌더마다 새로운 스타일을 입은 &lt;li&gt;가 렌더되면서 삭선이 첨가되었다가 사라지는 모습을 볼 수 있다. 토글의 상태. 이것을 관리한다는 의미에서 Satatefull한 컴포넌트라 부른다. state를 선언한 컴포넌트는 각자 이런 관리가 필요하다. 일부러 부모컴포넌트만 state를 가지게하면 자료의 일관성을 유지할 수 있다.   라이프사이클   리액트 컴포넌트 구조가 어떻게 관리받는지를 정리한 표이다. 녹색으로 표시된 지점이 우리가 별도의 로직을 넣을 수 있는 메서드를 구현하 수 있는 곳이다. 렌더 전후, 컴포넌트가 DOM에 등장한 시점이나 사라지기전 여러가지 로직을 집어넣기 쉽게 준비되어 있다. 재미있는 점이 있다면 state changed 뒤에도 업데이트를 하지 않는 케이스가 있다는 점이다. 리액트는 상태변화를 DOM에 반영할지 안할지 판단함으로서 바닐라 DOM보다 우월한 성능을 보장한다. DOM은 조금만 엘리먼츠를 바꿔도 전체를 바꿔야하는 약점을 가지고 있다.   없는 라이프사이클은 구현해서 쓰면된다. IDE에 따라 자동으로 넣어주기도 한다.   ","categories": [],
        "tags": [],
        "url": "/react-props-state-life-cycle/",
        "teaser": "/assets/images/HibikeQauntumSymbol_500x300_version_waifu.jpeg"
      },{
        "title": "AWS 서비스 S3, ES",
        "excerpt":"AWS 클라우드 서비스가 빠르게 성장하면 서비스 제공자가 서버를 소유하지 않는 서버리스 아키텍쳐가 많이 회자되는것 같다. 여기서 물리적 서버는 없을지 언정, 서비스를 제공할 그 무언가는 여전히 있어야 한다. 그 무언가를 볼려면 AWS 같은 곳에서 제공하는 컴퓨팅 제품을 보면 된다.   클라우드 컴퓨팅 제품 - Amazon Web Services  엔터프라이즈 앱, 클라우드 네이티브 앱 또는 모바일 앱을 구축하든, 인간 게놈의 서열을 결정하기 위해 대규모 클러스터를 실행하든, 조직의 구축 및 운영은 컴퓨팅으로 시작됩니다. AWS에서는 포괄적인 컴퓨팅 서비스…aws.amazon.com레거시 환경에서 클라우드로클라우드 사업자들은 레거시 인프라 패러다임에서 낭비되던 자원을 클라우드를 통해 묶는다. (서버든 네트워크 장비든) 그리고 그걸 1) 쪼개고 2) 사용자들에게 터미널과 3) 접근을 관리하는 도구를 만들어 제공한다.   사진만 보면 OS단이 사라진것 처럼 보이지만 사실 클라우드전용 OS가 있다. 이를 통해 자원을 효율적으로 쓰게 해준다. CPU를 예로 들었지만 HDD 저장공간도 서비스의 대상이다. 10만원 주고 사서 언제 고장날지 모르는 1TB를 10% 정도만 쓰던 시대가 끝났다.   이런 하드웨어적인 관점에서 이야기를 했는데 AWS가 제공하는게 이게 전부는 아니다. 멀리 떨어져있는 가상의 자원들을 잘 다루기 위한 도구들도 있다. 구글 Function, 람다AWS같은 서비스를 위한 서비스들은 서비스와 서비스 사이에서 교통정리를 할 수 있게 도와준다.   S3, EC2, RDS   그럼 클라우드에서 사용할 수 있는 서비스는 뭐가 있을까. 친숙해 보이고 흔한 3가지를 조사해봤다.   S3   정적인 자원을 관리하기 위한 제품인 S3는 버킷과 키라는 개념을 단위로 자원을 표현한다. 예전에 블로그에 사진을 올리고 사진의 URL을 커뮤니티 홈페이지에서 링크로 올렸던 시절을 기억하는 사람이 있을것같다. (나중에 CORS 때문인지 안되긴 했지만) 아마존이 그 시절은 기억하는건 아니겠지만 저장된 자원을 원하는 형태로 노출시키고 사용할 수 있는 서비스를 S3로 만들었다. 이걸로 정적인 웹서비스를 할 수 있다. 아직도 어떤 게시판에서 이미지 업로드를 지원하지 않는다면 S3에 이미지를 올리고 링크를 걸어보는것도 한번 시도해보고 싶다.   S3를 활용하면 이렇게 자원을 요청하고 받을 수 있다. 지금은 없는 URI라 실패했지만EC2   EC2는 S3보다는 이해하기 쉬운 서비스다. 기존에 많이 VMware같은걸로 윈도우에서도 많이 활용했었기 때문이다. EC2 인스턴스를 생성해보면 단순하게 OS에 접속할 수 있는 터미널을 제공한다. 들어가면 그냥 평소에 보던 CLI 가 보일것이다. 컴퓨터를 빌려쓴다는 초입부의 설명에 가장 일치하는 서비스는 바로 EC2일것이다.   스케일업, 스케일아웃을 지원하는 EC2. DB를 설치해서 쓰기도 한다.약간 IT덕후들만 알것같은 VMware가 아니라도 블루스택 같은 프로그램에서 가상화 OS에 자원을 할당하는건 이제 익숙한 풍경이 되었다. 최적의 CPU, 램, 렌더링 옵션 조합을 연구하는 게이머들 cheers!   자원을 객체처럼 찍어내고 할당하는 클라우드 시대의 풍경RDS   RDS는 EC2에서 나머지를 모두 빼버리고 특정 RDBMS 만 사용할 수 있는 터미널과 부가적인 컨트롤을 제공받을 수 있는 서비스라고 보면 될것같다. 우리가 돌아가는 환경에 대해 전혀 신경쓰지 않아도 이미 RDS 인스턴스가 생성될때 설치 및 유지관리를 위한 셋팅이 다 되어 있다. 사용자는 RDS의 주소와 계정정보만 알면 RDBMS를 쓸 수 있게 된다. DB를 보통 서버에 설치하고 관리할때 온갖 이슈가 터져나오는걸 회상해볼때 정말 편리하다. 암호, 보안, 백업, 이중화처리 등에 대한 Best Practice를 GUI에서 클릭으로 하게 됬으니 말이다. 스토리지 크기 또한 스케일링 된다. 머리아프게 리소스예측하는것도 이젠 그만 ( 대신 AWS Billing 계산 )   ","categories": [],
        "tags": [],
        "url": "/aws-s3-es/",
        "teaser": "/assets/images/HibikeQauntumSymbol_500x300_version_waifu.jpeg"
      },{
        "title": "2019년 8월 26일 TIL",
        "excerpt":"1. 프로젝트에 바로 참여할 수 있을지 평가하는 날이었다. 재밌어서 ‘이런거 매주에 한번 하면 좋겠는데?’란 생각도 들었지만 정성적 평가를 못하는 경쟁을 하게되면 대외적으로 보이는것만 신경쓰는 문제가 생긴다. 좋은 설계나 데이터의 확보 보단 요구되는 스펙만 맞추는것이나 뭔가를 복사하는데 끌리게 된다.     exports 를 잘 활용하지 못했다. module.exports.* 의 패턴으로 사용하라고 권장이 되어있길래 그것만 쓰다가 대응을 바로 못했다. 어떻게 돌아가는지는 아는데 거기로 집어넣는 과정에서 헤매니 환장.   Http에서 클라이언트로 파일 내려주는데 40분 정도 소모했다. JSON만 쓰다가 적응을 못했다. Buffer를 쓰는걸로 해결은 했지만 그게 뭔지 아직도 정확히 모르겠다. 그리고 다시 확인해보니 Buffer를 안써고 response에 바로 담아도 된다. 환장. 파일을 반환시켜주면 Content-Lenth 헤더를 Http가 당연히 써주겠지라고 생각한게 오착.   file-system 을 활용하는데 Path를 지정하는데서 환경변화에 대응을 잘 못해서 20분 정도 낭비를 했다. 경로의존적인 것엔 __dirname 쓰면 된다.   CORS 처리를 서버에서 해도 브라우저단에서 막는 문제가 있다. 확장앱을 안켜고 크롬 Security 모드도 안끄고 처리해보려고 fetch에 mode:no-cors 옵션을 활용하려다가 이렇게 쓸 경우 다른 종류의 응답이 오고 프로미스단에서 에러가 난다. 다르게 처리를 해야한다는 사실을 늦게 깨달아서 또 시간을 소모했다.   React-Router를 처음 썼는데 설계의 패러다임이 또 바뀌는것 같다.   이미 써본것도 깔끔하게 한 큐에 안되고 한 두번 정도 버벅거리는게 전체적으로 작업진행속도를 늦췄다. 빠르게 제대로 하는 방법이 어디에 있지 않을까 고민은 해보지만 왕도는 없는것 같다.   ","categories": [],
        "tags": [],
        "url": "/2019-8-26-til/",
        "teaser": "/assets/images/HibikeQauntumSymbol_500x300_version_waifu.jpeg"
      },{
        ...
        "excerpt":"   메일 서비스로 서버를 구성하는 방법에 대한 글을 읽었다. SES라는 이름으로 AWS에서도 사용할 수 있다. 메일 서버를 운영하는 곳에메일 포맷의 데이터를 전송하는 nodemailer로 토큰을 발행하고 토큰이 인증 URL로 요청되면 값을 검증해서 사용자의 가입을 승인하는 구조다.   Atom 에디터를 마크다운 전용 에디터로 쓰기로 했다. 기존 Notes에 있던 내용을 전부 .md파일로 옮기고 커밋도 해가면서 .md로 리팩토링하면서 쉬었다. 내용도 읽으면서 다시 복습도 했다. 필요없는 내용이 추려지면 md파일을 퍼블릭한 github에 올리려고 한다.   개발자로서 삶과 사이드프로젝트, 창업 이런 키워드로 세션을 하나 들었는데 중요하다. 지금당장보다는 지금 부트캠프가 끝나면 훨씬 더 중요한 정보다. 여기서 기억에 남는 문장 — ‘크고 부실한 프로젝트보다는 작고 단단한 프로젝트가 더 의미있다’   #Culture      ‘청춘돼지는 꿈꾸는 소녀의 꿈을 꾸지 않는다’ 영화를 봤다.            이 픽션은 스케일이 굉장히 크다. 상대성이론을 운운하면서 시간여행도 하고 양자역학적인 아이디어를 거시세계에 적용한 걸 소재로 한다.       예를 들면 인간을 양자처럼 취급해서 관측하는 시점에 존재가 확정된다.       한마디로 이해하기가 힘든 ‘큰’ 이야기다. 그래서 잘 소화시키기가 힘든 이야기였는데 실패한 영화였다. (소설은 훨씬 좋으리라고 생각한다)           그렇게 보고 집에 오면서 오늘 들은 세션이 떠올라서 다짐했다.      “단단하고 작은 프로젝트를 해야겠다. 쓰레기를 만들면 안되겠다”   ","categories": [],
        "tags": [],
        "url": "/2019-8-27-til/",
        "teaser": "/assets/images/HibikeQauntumSymbol_500x300_version_waifu.jpeg"
      },{
        "title": "2019년 8월 28일 TIL",
        "excerpt":"#Code      redux 입문  전역에서 사용가능한 state를 얻었다는 기분이 드는데 이 state는 업데이트를 해도 리액트의 렌더는 일어나지 않는것 같다. Mount, State도 관련이 없는 독자적인 일이니까 당연한 일이다. (그런데 이건 react 기준이고 또 react-redux를 쓰는 지금은 자동으로 뭘 하고 있을지도 모르겠다. 구독같은 기능으로 독자적으로 처리를 하고 있다.) 그래서 나름 뭔가 바꿨는데도 업데이트가 되지 않는 문제가 발생했다. 강제 렌더링 명령이 있다는데 ‘강제’라고 하니 쓰기 싫고 자바스크립트로 수동으로 DOM을 조작하지 않고 바꿔보려고 하다가 오늘은 일단 작업끝.   React router, redux를 함께 쓴다면 어떤 설계를 해볼 수 있을까. 그렇게 될 경우의 수가 복잡해지는것 같다. 이전에 그냥 데이터를 흘려보낼것만 생각해서 컴포넌트를 짤 때는 코드는 복잡해도 설계는 단순했던것 같은데. 그렇다면 router가 최우선이 되어야 할것같다. URL로 사용자가 원하는 데이터가 보이는게 보장이 되어야 하니까 말이다.   redux는 DOM을 한번에 render하는 한다고 하는데 그러면 리액트의 장점. 최대한 DOM렌더를 억제하는 기능과 대치하는것 아닐까? 검색해보니 리듀서에서 퓨어함수를 강조하는 이유가 바로 render 이슈 때문이다. 그리고 그렇게 성능에 영향을 미치지도 않고 디버깅이나 구조개선에는 너무나 압도적 이득을 주기 때문에 안쓸이유가 없는듯하다. 하나의 기능을 위해 몇 단계의 추상화를 거치는게 번거로울뿐.        직접 수정을 하면 안되는 객체들을 다루는 방법으로 object.assign를 시도 했는데 뭔가 익숙하지 않아서 한시간 정도 헤매다가 결국 비구조할당으로 돌아왔다. 내일은 immutable.js를 시도해볼 예정이다. Nested 한 구조에선 쓸 수 밖에 없을 것같다. #Life       한달넘게 엄청 푸시하면서 공부하다보니 요즘 낮에 체력이 바닥이 되어가는 기분이 점점든다. 오늘부터 7시간 잘거고 질펀하게 운동도 한번   ","categories": [],
        "tags": [],
        "url": "/2019-8-28-til/",
        "teaser": "/assets/images/HibikeQauntumSymbol_500x300_version_waifu.jpeg"
      },{
        "title": "2019년 8월 29일 TIL",
        "excerpt":"   Redux Todo 를 완성했다. UI도 항상 손수 CSS를 하다가 오늘은 ant-design을 적용시켜봤다. 리액트 컴포넌트를 준비해서 주니까 퀄리티도 속도도 엄청나게 개선됬다. 처음으로 볼만한 UI를 완성시킨것같아서 기분좋았다.   버그가 많은 경우 props나 액션의 이름에서 나는것 같다. 잘못 지정을 해줘도 에러가 나지 않고 그냥 반쯤 동작을 하다가 말뿐이고 뭘 찍어봐도 undefined가 뜰뿐이니 원인을 찾기가 쉽지가 않다. 디버깅을 용이하게 할 네이밍방법이나 코등습관도 중요하겠지만 아마 이런 문제를 해결하기 위한 방법이 있을것같다.   사실 아직 익숙하지 않아서 그런거고 또 하루 정도만 익숙해지면 속도가 나니 반복하면 좋아질것 같다.   처음엔 CSS가 너무 불편하고 그랬는데 반나절 정도 붙잡고 씨름하는걸 3–4번 정도 하고 나니 대충 어떤 문제가 생겼을때 원인을 추적하고 적당한 솔루션을 찾는 속도가 빨리 붙는것 같다. CSS를 하면서 제일 답답한게 뭘 바꿔도 아무것도 바뀌지 않을때가 참 많다는 건데 그 안되는 이유를 하나하나 예측할 수 있게 된것 같다. 여러가지 베스트 케이스를 눈여겨봐놨다가 나중에 재치있게 짜는것도 조금씩 하기 시작했다. padding, margin, border-radius, flex 하나씩 유연하게 쓸 수 있게 되었다.   ","categories": [],
        "tags": [],
        "url": "/2019-8-29-til/",
        "teaser": "/assets/images/HibikeQauntumSymbol_500x300_version_waifu.jpeg"
      },{
        "title": "2019년 8월 30일 TIL",
        "excerpt":"#code      MergeSort 알고리즘을 풀었는데 예상보다 잘 풀렸다. 처음에 재귀함수를 접할때 어쩔 줄 몰라했던걸 생각하면 큰 진전이다.   오늘 처음으로 팀프로젝트를 시작했다. 프로젝트를 하기 전에 해야할 많은 설계들과 팀원들이 가진 아이디어를 통합하고 팀이 지향하는 바를 합의하는데 많은 시간이 들어갔다. 설계한 대로 제대로 될지는 미지수지만 실패를 해도 이런 과정 속에서 제대로 협업을 하는 것이 어떤것인지 알아가는 과정이 될 수 있으리라 기대한다.   지금까지 배운것들을 잘 활용하는걸 프로젝트의 뼈대를 삼았고 거기에 챌린지한 요소로 서버에서 데이터를 가공하고 스케쥴을 돌리는것과 타입스크립트를 적용하는걸 계획했다. 익숙한걸 조금씩 다르게 복습하는게 중요하다는 생각이 든다.        기술스택을 조사하다가 ElasticSearch, Redis, TypeScript, GraphSQL 같은 이름들이 많이 보였다 .백엔드에 데이터 과학적인 관점을 요구하는 구인들도 많이 있었고 앞으로 많이 많이 배울것도 있고 해볼수 있는것도 많아서 기대된다. #life       목이 급격하게 아프기 시작해서 노트북 스탠드도 사고 트랙패드, 기계식키보드도 마련을 했다. 비싼 액세서리이다 보니 이 컴포넌트들을 잘 활용하려면 다음은 아이맥이 필요하겠다는 생각이 든다. 미리 샀으면 좋았을걸. 건강에는 미리 투자해야겠다.   ","categories": [],
        "tags": [],
        "url": "/2019-8-30-til/",
        "teaser": "/assets/images/HibikeQauntumSymbol_500x300_version_waifu.jpeg"
      },{
        "title": "2019년 8월 31일 TIL",
        "excerpt":"   프로젝트 2일째, 업무를 분담하고 Server/Client의 기본적인 프레임을 짰다. API문서를 좀 더 치밀하게 짜야할것 같긴한데 하다보면 또 내용이 바뀔것 같기도 하고 어느게 적정선을 찾기가 힘들다. 각자 의견을 존중해 프론트랑 백을 구분하지 않는데, 분업경험을 유지하기 위해 자신이 서버로직을 만들고 자신이 요청하지는 않도록 과업을 배분했다.   React Router를 사용하니까 분업의 그림이 그려졌다. 이게 없으면 어떻게 했을까 걱정이 된다. redux가 컨플릭트를 예방해줄 수 있을거란 생각이 들었었는데 (이번에 도입은 안했다) 이렇게 하나씩하나씩 쓰다가 SAGA같은것도 쓰게 되는걸까.   원래는 TYPEscript를 쓰고 싶었는데 이번에는 아는 기술을 견고하게 단련하고 협업경험을 쌓는다는 목적에 집중하기위해 포기했다.   뭔가 코드를 계속 치고 싶은데 이것저것 설정하고 한번에 처리할 방법을 고민하는게 나는 살짝 마음에 안들었다. 필요한 비즈니스 로직만 계속 칠 방법은 없을까. PM일도 살짝 싫다. 관리가 재미를 주는 지점도 있긴하지만 역시 뭔가 두들기고 있을때가 확실하게 즐겁다.   목이 뻐근하던게 이제는 어깨로 내려오기 시작한다. 그래도 택배로 도착한 장비들로 디스플레이 눈높이도 맞고 꾸준히 스트레칭도 하고 일단 고개를 들고 있는 시간을 좀 줄이면서 차후를 봐야겠다.   ","categories": [],
        "tags": [],
        "url": "/2019-8-31-til/",
        "teaser": "/assets/images/HibikeQauntumSymbol_500x300_version_waifu.jpeg"
      },{
        "title": "2019년 9월 3일 TIL",
        "excerpt":"   프로젝트는 상상한대로만 굴러가지 않는다. 그럼 해야할건 뭘까. 상상대로 굴러가지 않아도 괜찮은 방법을 선택해야 한다. 어떻게 하면 상상대로 굴러가지 않아도 조급하지 않은 마음으로 지치지 않는 마음으로 목적을 달성할 수 있을까.   일단은 중요한 일을 구분지어서 전체적인 흐름을 달성시키고 목적을 달성가능한 수준으로 다시 재설정하는게 첫 단계일것같다. 초조해지고 절망에 빠지지 않도록.   ","categories": [],
        "tags": [],
        "url": "/2019-9-3-til/",
        "teaser": "/assets/images/HibikeQauntumSymbol_500x300_version_waifu.jpeg"
      },{
        "title": "2019년 9월 4일 TIL",
        "excerpt":"   과업을 선정할때 사용자가 필요한 부분부터 하는게 먼저라는 원칙을 다시 또 배웠다. 어제는 NAMSE 개발자분, 오늘은 박근핵닷컴 서버 이야기를 들으며 든 생각이다. 그래야 만드는 사람도 즐겁고 쓰는 사람도 즐거울 수 있는것같다. 이 원칙에서 어긋나는 것들은 대부분 있어보이고 싶다는 생각이거나 내가 배운걸 실전에서 써먹어야 겠다는 생으로 오버 엔지니어링하면서 생겨났었다.   같이 일하게 무섭다. 일심동체가 되어서 서로의 작업 영역을 건들지 않아야 하는데 그런게 가능한건지. 기준을 혼자서 높게 잡은건가.   충돌을 내지 않고 또 PR을 했을때 알아보기 보기좋고 남들이 유지보수하거나 고쳐쓰기 좋은 디자인 패턴을 찾아내야할것 같다. diff를 볼때 파악이 안되서 리뷰를 하기가 힘들다.   ","categories": [],
        "tags": [],
        "url": "/2019-9-4-til/",
        "teaser": "/assets/images/HibikeQauntumSymbol_500x300_version_waifu.jpeg"
      },{
        "title": "2019년9월7일 TIL",
        "excerpt":"   회사협업 이야기를 들었다. SCM관련 프로젝트를 맡긴다는 이야기를 듣는순간 살짝 갸우뚱.   오늘은 이 코드 때문에 애를 많이 먹었다. let list = [“태훈”];let result = {};list.forEach(ele =&gt; {result[”” + ele] = (Math.random() * 100).toFixed(2)})console.log(result)3. 프로젝트를 할때는 적당한 러닝커브가 있어야 한다고 생각한다. 당장 돈 받는것도 아닌데 너무 보수적으로 하다간 피본다. 그래서 따지고 보면 그래서 지금은 지겨울 정도로 잘하냐?라고 물어보면 그것도 아닌데. 익숙하기만 하고 못하는 상태를 견뎌내는것도 노력이고 투자라고 생각해야겠다.   ","categories": [],
        "tags": [],
        "url": "/2019-9-7-til/",
        "teaser": "/assets/images/HibikeQauntumSymbol_500x300_version_waifu.jpeg"
      },{
        "title": "2019년 9월 9일 TIL",
        "excerpt":"   오늘은 다사다난했다. 격렬하게 스프린트 회고도 하고 5시간 정도 같이 모여서 배포랑 디버깅, 포스트맨의 활용을 연구하면서 쿠키공부도 하고. 이런 시간들이 그냥 코드치는것보단 재미있는것 같다.   포스트맨을 연구했다. 포스트맨의 테스트모드로 들어가서 셋팅해놓은 쿠키를 사용하지 않음으로 체크해놓으면 가입부터 서비스 그리고 로그아웃까지 모든 API를 돌려보면 다 테스트를 해볼 수 있어서 참 보기도 좋다. API문서로 출력도 된다. 셋팅해놓은 쿠키를 쓰는것도 개별 기능에서 세션ID를 가정하고 테스트하기에 유용하다. 설정해놓은 리퀘스트를 공유할 수도 있으므로 필수적인것같다.   쿠키흐름에 대해 공부해봤다. 서버가 판단하기에 쿠키를 내려주기에 적당하다고 생각되면 set을 키로 가지는 쿠키 객체가 내려간다. 그럼 다음 통신에서 클라는 그 쿠키를 그대로 들고간다. 물론 그 쿠키를 들고갈지는 클라이언트 단에서 결정할 수 있다. credential 옵션으로. 이런 흐름은 철저히 CORS와도 연관이 되어 있어서 언제든지 통신환경이 변할때마다 언제든지 튀어나 올 수 있다는 점이 흥미롭다. (까다롭다)   SQL쿼리를 짜는데 오늘 시간을 좀 썼다. MYSQL의 경우 지원해주지 않는 함수가 꽤나 되다보니 그걸 억지로 구현하면 쿼리가 금방 지저분해진다. 오늘은 3개 테이블을 풀조인해야하는 일이 생겼는데 좀 처럼 감이 오지 않았다. 나는 풀조인이 익숙하다. 풀조인은 포기하고 느낌가는 방향으로 3개를 조인해놓고 WHERE로 하나씩 소거해가면서 내가 원하는 결과값만 나오는 방식으로 만들어보는 방향으로 진행을 했다가 실패. 결국 차분하게 말로 수두코드마냥 문장으로 쓴다음 문장을 SQL로 옮겨쓰는 전략을 해봤는데 그게 통했다.   사실 ORM을 쓰고 있는데 일부러 쿼리를 짜게 된건 ORM DDL이 잘 셋팅이 안되서다. 쿼리를 다 짜놓고 나니 호기심이 생겨서 다시 ORM 셋팅을 뒤집어보고 붙여보고 질문도 하고 하다가 이게 처음에 DDL을 실행할 순서가 꼬여있다는 결론에 이르렀다. exports할때 Association할 대상들이 제대로 캐쉬에 올라와있지 않아서 에러가 발생 했었던것. 더군다나 그런 문제를 회피하기 위한 모듈실행 패턴들이 있었는데 제대로 구현을 안해놔서 지금까지 그냥 놀고 먹고만 있었던 코드들을 발견했다.   프로그래밍을 계속 고생하면서 감을 유지켜야하는 영역을 만나게 되는것 같다. 어설프게 여기저기서 익히고 잊고 배우고 잊고 하면 나도 괴롭고 현실도 괴롭다. 결국 어딘가 구멍을 깊게 파야할 시점이 오는것같다. 그래서 풀스택이란 말이 참 멀게 느껴진다. 나는 뭘까..?   ","categories": [],
        "tags": [],
        "url": "/2019-9-9-til/",
        "teaser": "/assets/images/HibikeQauntumSymbol_500x300_version_waifu.jpeg"
      },{
        "title": "2주 프로젝트 회고",
        "excerpt":"        처음엔 아이디어가 재미있었고 내가 의미있다고 생각했었다. 엄청나게 많은 기획을 하고 여러가지 기능들을 상상해냈다. 그렇지만 삼일 만에 현실적으로 무엇을 할 수 있는지 측정하고 청사진을 수정해나가는 작업을 시작했다.            그러다 중간부터 작업이 지루하게 느껴졌다. 지금 하는 작업을 어떤 참고문서도 안보고 버그도 없이 진행할 정도로 내것이 된건 아니지만 클라이언트에서 요청하고 서버에서 요청을 컨트롤하고 DB가 상태를 관리한 구조가 틀이 잡히자 뻔하다는 생각이 들었다. 내 프로젝트는 그냥 CRUD 노가다 프로젝트인가. 다른 팀은 AWS에서 새로 서비스를 프로젝트에 붙이거나 SSL같은 이슈들을 접하고 고민하는게 보일때마다 마음이 복잡했다.       3.그냥 복습하는 프로젝트란 말을 곧이 곧대로 받아들여서 정말 새로운 스택하나 없이 들어온게 후회가 됐다. 냉정하게 보면 여기서 뭘 넣든 오버엔지니어링이다. 그러나 억지로라도 그런걸 하는게 학습프로젝트의 의미 아닐까 생각이 든다. 프로젝트를 하면서 각자가 만족할 수 있는 새로운 학습과제들을 선정할 수 있어야 한다는 결론을 내렸다. (그러나 실제로 서비스를 하는데 집중하다보면 오버엔지니어링도 할 틈이 없을것이다. 우리팀을 포함해서 여러 팀들이 토큰을 쓰긴했지만 주고 받기만 했지 엄격한 보안을 구현하진 않았다. 가시적이지 않아서 그랬던것 같다.)           내가 만족하는게 쉽지가 않은것같다. 프로젝트가 사용 가능한 궤도에 오르자 왜 그렇게 이걸 만들고 싶었을까 생각이 들었다. 그러나 시간이 지나면서 점점 애정이 들고 이런건 다른곳에서 본적이 없고 특이한 구석이 많다는 확신이 드니까 괜찮아졌다. 이머시브 과정이 끝나면 나의 사이드 프로젝트로 이어나가야겠다는 계획이 섰다. 저작권은..(?)            프로젝트를 시작하기 전에 내가 마일스톤으로 삼은게 있다. 바로 데모가 지루하지 않고 발표하는 사람들이 생기가 있었으면 좋겠다는 것이었다. 그래서 그런 느낌을 목표로 PT를 준비했는데 2주 프로젝트 서비스 자체가 사람의 재치를 발휘할 수 있는 서비스라서 나름 재미있게 발표를 한것같다. 나 혼자 재밌었는지는 모르겠지만 (팀원들이 재치있는 글을 준비했다)            마음이 괴롭고 그래도 어떻게 추스려서 멈추지 않고 잘 달린 프로젝트였다. 뭔가 나에 대해 배운게 있다면 1) 별 수 없는 자책을 한다. 2) 제대로 하지도 못하면서도 지금하는게 뻔하다는 마음을 품는다. 그런게 내 심리적 약점이란걸 알 수 있었다. 이미 이건 잘 알고 있긴 했다. 남들과 다른 길을 걷고 싶다면 그게 남이 감동할 수준이 되어야한다. 쉬운게 아니다.       4주 프로젝트를 시작하며      프로젝트 구성원들이 만약 지루하다고 느낀다면 인위적으로 오버엔지니어링을 해야겠다. 물론 때 이른 걱정이다 . 어차피 이번에는 새로운 스택들이 들어간다. 리액트 네이티브부터 타입스크립트, 파이썬, AWS, d3.js 지루할일은 거의 없을것 같다. 이번에 사용자 있는 프로젝트를 하겠다고 큰소리를 쳐 놓은 상태에서 새로운 스택을 쓰는게 약간 불안하긴 하지만 하지만 일정이 늦어지더라도 학습 프로젝트의 성격은 지키는게 맞다고 판단한다.   문서를 잘 써야 한다. 구성원들이 동의한 합의점이 무결성을 유지하면서 쉽게 조회할 수 있는 문서체계를 갖추어야한다. 끝없이 프로젝트의 비전에 대해 팀들과 잡담을 나누면서 조금만한것까지도 고민할 볼필요가 있다. 어차피 코드만 너무 짜는것도 프로젝트 전체를 볼때 의미가 크게 없다. 코드를 10% 더 많이 적는것보다 10% 더 프로젝트 방향과 깊이에 대해 고민하는게 더 큰 도움이 될것 같다.   이번에도 재미있고 활기있는 PT를 하고 싶다. 왜 그런 목표에 끌리는지 모르겠는데 4주 프로젝트 발표하는 사람들이 항상 장례식에 온 표정을 하고 있어서 늘 불쾌했다. 그런 컨디션으로 코드를 어떻게 짜는걸까?   ","categories": [],
        "tags": [],
        "url": "/2/",
        "teaser": "/assets/images/HibikeQauntumSymbol_500x300_version_waifu.jpeg"
      },{
        "title": "2019년09월16일 TIL",
        "excerpt":"   오늘은 팀이랑 첫 오프라인 미팅을 했다. 어떻게 협업을 할지 툴을 공유하고, 프로젝트의 서비스 플로우에 대한 비전을 맞춰나가고 문서로 작성해나가는 작업을 하루종일 했다.        스택이 정해졌다. 사용자가 임의의 형태로 지정한 자료를 업로드하는 서비스를 구상했는데 어떻게 변화무쌍한 자료를 처리할지 고민이 생겼고 NOSQL이 채택되었다. 리액트 네이티브를 일단 쓰되 모바일만 고려해서 심도 있게 짤지 웹뷰를 통해 Desktop을 지원하는 범용적 서비스를 만들지 고민했고 웹뷰를 선택했다. 그 외에 인프라쪽에서도 Route53, CDN, AWS-Recognize 등 프로젝트 성격에 맞는 서비스들을 도입하기로 했다. #life       집이 단수가 되어서 어제 오늘 씻지도 못하고.. 급한대로 변기뚜껑안에 저장된 물을 쓰는 해프닝이 있었다. 아까전에 고쳐졌다는 연락이 왔는데 기적이라고 밖에 볼 수 없는 문명의 혜택을 다시금 회고하는 시간이었다. (피터슨은 문명 인프라의 기적성을 제대로 볼 줄 알고 감사해야한다고 강조하는데 나도 이제 체화가 되었다.)   ","categories": [],
        "tags": [],
        "url": "/2019-09-16-til/",
        "teaser": "/assets/images/HibikeQauntumSymbol_500x300_version_waifu.jpeg"
      },{
        "title": "2019년 9월 18일 TIL",
        "excerpt":"   프로젝트 구현에 필요한 학습량을 약간 줄였다. 뭐 지금 쓰는것도 잘 못하는걸 상기하면서 반성하면서 한달을 진행해야겠다.   Django, Rest-Framework를 자료를 봐가면서 구현했다. secret 정보를 네임드스페이스에 집어넣고 나중에 꺼내쓰는것도 구현했다. 모든게 익숙하지 않아서 한단계 넘어갈때마다 재밌다.   원래 Atlas에 DB를 빌려쓰려다가 TLS를 요구해서 EC2에 설치해서 쓰기로 했다. 설치가 제대로 안되서 mongod.conf에 집어넣은 설정을 제대로 못읽는 문제가 있긴한데 이래저래 부딪치면서 결국 돌아가게 하는데는 성공했다.   RestFramework  앱안에 여러가지 앱이 있을 수 있는 점에서 특이했다. 폴더구조가 어플리케이션의 설계에 크게 영향을 미친다는게 약간 깝깝하기도 하다. 아직 로직이 안들어갔지만 S3에 업로드하고 자원의 주소를 DB에 넣는 작업을 하려고 했는데 어떻게해야할지 감이 안온다.   ","categories": [],
        "tags": [],
        "url": "/2019-9-18-til/",
        "teaser": "/assets/images/HibikeQauntumSymbol_500x300_version_waifu.jpeg"
      },{
        "title": "2019년 9월 19일 TIL",
        "excerpt":"   장고 갈아엎고 플라스크로 다시 시작했다. 장고에서 rest-framework와 mongoenine이 묶여있는걸 사용했었는데 나중에 알고보니 3년동안 갱신이 없었던 프로젝트였다. 그래서 마음이 불안불안하던차에 django에서 만든 모델들이 반영이 안되는데 특정 필드이름만 인식되는 문제가 생겼는데 해결책이 안보여서 좀더 직관적이고 하이코드에 가까운 flask로 바꿔탔다. 3시간만에 장고로 헤매면서 구현한곳까지 구현했다. node.js에 익숙하다면 첫 웹서버는 flask다.   무척 피곤해졌다. 그 동안은 11시반까지 하면 오래한거라 생각하고 집에갔다. 바로 잠은 안자더라도 쉬는 시간이 있었다. 지금은 12시 30분까지 거의 한계까지 몰아붙이고 해서 집에가면 그냥 씻고 불을 꺼놓고 선풍기로 바람으로 말리다가 마르면 잔다. 이렇게 일주일을 하니 머리가 약간 뇌졸증이 올것마냥 살짝 지끈거린다. 아무래도 이런 라이프스타일은 오래할게 못되나보다. 구로등대에서 과로사하는 사람의 컨디션이 이런걸까하고 뭔가 졸리는건 아닌데 머리가 멍하다. 컨디션 회복될때까지 9시10분에 땡치고 집에가서 영화나 봐야겠다. 번아웃오기전에 회복해야지.   ","categories": [],
        "tags": [],
        "url": "/2019-9-19-til/",
        "teaser": "/assets/images/HibikeQauntumSymbol_500x300_version_waifu.jpeg"
      },{
        "title": "2019년 9월 20일 TIL",
        "excerpt":"   OAuth 를 구현하려고 여러가지를 알아봤다. 토큰을 가지고 여러가지를 해볼 수 있는거라 간단한 작업만 하려고 한다면 쉽게 사용할 용도로 만들어진 라이브러리를 써서 한없이 쉽고 어렵다면 한없이 어려운 이슈였다.        파이선 모듈과 패키지, 동작환경에 대한 이해가 부족해서 폴더구조가 깔끔하지 못하고 코드가 깔끔하게 보이지 않는게 불만이다. 지금 당장은 기능이 급해서 무시하지만 점점 익숙해지면 처리할 기회가 생길것같다. 실행환경이 꼬였을때 생기는 에러는 정말 불편하다. 개인적으로 코드스타일은 파이썬이 마음에 들지만 pip은 npm처럼 package.json만큼 편리하지 않은것 같다. #life       몇 일 연속으로 늦게까지 남아서 작업을 하다보니 엄청 피곤해져서 오늘 낮엔 도저히 안되겠다 싶어서 낮잠을 잤다. 잠자는 양은 똑같았는데 문제가 생긴걸 보면 아무래도 집에서 긴장을 풀고 있는 시간이 필요한것 같다. 오늘은 3시간 빨리 집에가서 맥주라도 먹고 영화라도 봐야지.   ","categories": [],
        "tags": [],
        "url": "/2019-9-20-til/",
        "teaser": "/assets/images/HibikeQauntumSymbol_500x300_version_waifu.jpeg"
      },{
        "title": "2019년 11월 21일 TIL",
        "excerpt":"   실수로 깃이 꼬여서 origin에 있는걸로 다시 시작하려고 하다가 secret.json으로 파일을 관리하던걸 지워버렸다. 따로 백업할 방법도 생각하지 않고 쓰다 보니 삭제 되버렸다. 여기저기서 복사해서 쓰던 토큰키들을 다시받는건 어려운건 아니지만 rm을 이제는 쓰지 말아야겠다. 다짐한다. 무조건 백업한다. 조금 안깔끔해도 괜찮아.   이번 프로젝트는 S3에 파일을 쓴다. 처음에 서버에서 다 처리하려고 하던걸 클라이언트에서도 하는걸로 로직을 변경했다. 처음에 안 알아보고 다 서버에서 하는게 안전하다고 생각하던게 착오였다. 처음에 공부하는 시간을 30분씩 가지면서 소프트웨어 스택을 구성할걸 그랬다. 그냥 당연히 상식이란걸 믿고 하다가 작은 삽질을 했다.        파이선은 모듈관리가 너무 불편하다. requirement에 내가 써주지 않으면 관리도 안된다. 자동화 하도록 스크립트를 짜야하나. 뭐 날릴때 마다 빨간게 두렵다. #life       어제 두시간 반 정도 일찍 들어가서 맥주 먹고 좀 긴장을 풀다가 잠들었는데 훨씬 몸상태가 좋다. 아무래도 잠보다도 뭔가 긴장을 푸는게 필요한것 같다.   ","categories": [],
        "tags": [],
        "url": "/2019-11-21-til/",
        "teaser": "/assets/images/HibikeQauntumSymbol_500x300_version_waifu.jpeg"
      },{
        "title": "2019년 9월 23일 TIL",
        "excerpt":"   인증키를 통해서 클라이언트단에서도 S3로 파일을 올리고 서버에서도 S3와 통신을 하게되었다. OAUTH를 통해서 여러가지 인터넷사업자들의 정보가 손쉽게 연동이 되는걸 보면 그럼 도대체 나는 어떤 개발자가 되어야하는걸까 두려워진다. AWS congnito로하면 훨씬 단단하고 범용성있는 모듈을 안쓰는 대신 돈을 끼려고 자체적인 서버로직을 만드는 사람인걸까. 아니면 오픈소스로 나와있는걸 가져다가 조립하는 사람일까. 개발자의 미래는 높은 분들이 만든 논리를 조립하는 사람이 되는건가. 나는 그게 비참하지 않고 즐길 수 있고 사람들이 좋아할 수 있는 일이기를 바란다.   AWS는 자원을 관리하는 기술이다. 그리고 그게 하나하나가 다 돈이다. 그리고 전문가가 아닌 입장에선 그걸 깊게 이해하는데 한계가 있다. 때문에 이것도 Oracle 엔지니어링처럼 사용자에 맞게 고쳐주는 서비스의 수요가 있을것같다. 오랫동안 살아남을 기술이고 전문화되면 꽤나 괜찮지 않을까. 마치 회계처럼 말이다. 그 원리와 필요성은 이미 아주 단단하게 잡혔지만 그 컨트롤은 아주 사람들마다 숙련이 달라서 계속 재밌게 도전할 수 있는 기술. 나도 흥미롭긴 하지만 아직은 이머시브가 끝나지 않은 입장에서 자신의 가능성을 쳐내는건 두렵다.   ","categories": [],
        "tags": [],
        "url": "/2019-9-23-til/",
        "teaser": "/assets/images/HibikeQauntumSymbol_500x300_version_waifu.jpeg"
      },{
        "title": "2019년 9월 25일 TIL",
        "excerpt":"   Spint2 가 끝났다. 6일을 했는데 너무 길었다. 도대체 우리가 뭘하고 있는건지 어디로 가는지 합의된 목표의식이 팀에 있어야 했는데 그냥 기계적으로 코드만 짜고 있었던것 같다. 이제 스프린트는 무조건 4일 단위로 하기로 했다. 처음에 생각한 목표가 흐려지더라도 다시 붙잡을 수 있도록! 이게 중요한것 같다.   CORS문제가 또 발목을 잡는다.        OAuth를 한다는건 단순히 로그인만 하는게 아니었다. 대형회사들의 체계에 편입되는 것이기도 한것같다. 과연 나는 어떤 개발자가 되어가는 건지 많은 생각이 들었다. 벽돌을 조립하고 있는게 개발자인건가? NAMSE 님이 개발자를 요리사에 비교한것이 자꾸 생각난다. 나는 사람들에게 요리를 해주는 요리사가 되고 싶은 걸까. 요리사를 위한 작물을 만드는 농부가 되고 싶은걸까. 둘다 마음에는 있다. 하나는 단단한 벽돌로 가는 길이고 하나는 화려한 표현자가 일인것 같은데 말이다. #life       지금 당장 큰 문제는 낮에 100% 컨디션이 안나온다는 점이다. 사람이 문제를 잘 해결하려면 같은 방식으로만 문제를 자꾸 풀려고 해서는 안된다. 지금 문제를 푸는 방식이 제대로 된건지 스스로 메타적 사고를 할 수 있어야 하는데 피곤하면 이게 안되고 자꾸 안되는 방법을 자꾸 시도하게 되는것 같다. 컨디션을 최상으로 끌어올릴 필요가 있다.   밤이되면 약간 좋아지고 작업이 몰입되기 시작해서 늦게까지 하는게 반복된다. 이제는 이 악순환을 진짜 끊어야될때가 왔다. 11시 되면 슬랙에 이제 작업 그만하라고 다음날 술취한 기분으로 작업하는 일없어야 한다고 질리게 말할 예정이다. 이런 강행군도 부트캠프니까 할 수 있는거니까. 재밌게 즐겨봐야지.   ","categories": [],
        "tags": [],
        "url": "/2019-9-25-til/",
        "teaser": "/assets/images/HibikeQauntumSymbol_500x300_version_waifu.jpeg"
      },{
        "title": "2019년 9월 26일 TIL",
        "excerpt":"2019년 9월 26일 TIL      그냥 아무생각 없이 잘하겠거려니 하고 생각하고 있다가 팀원이 기대한거랑 다른걸 들고와서 쪼매 화가 났다. 코드 리뷰하면서 뭔가 안풀리는거 있으면 같이 풀고 잘 보이지도 않는 코드 설명이나 하다가 정작 중요한 개발 방향에 대한 검증은 하나도 못했다고 생각하니 삽질한 기분이다.   그래서 대안은 매일 5분이라도 완성된 솔루션을 보면서 지금 안된게 뭔지 해야될게 뭔지 방향성을 공유하는 작업을 하는것인것같다. 매일 아침에 해야지.   사람은 각자가 생각하는게 다르다. 서로간의 코드가 다르면 코스트를 지불해서 맞춰야 한다. 다양성이 제공하는 통찰의 힘도 있지만- 공짜는 없는 셈이다. 코드가 다르면 누군가는 그 다른 코드를 맞추기 위해 비용을 지불하지 않으면 목표달성에 실패할 수 밖에 없다고 생각한다. 그 지불의 대상이 그게 내가 되야할 수도 있고 다른 사람일수도 있고. 아무도 지불하면 그냥 망하겠지. 양쪽다 지불하면 최상일것이다. 그래서 부부는 서로가 끝없이 서로에게 억울하지 않기위해 노력해야할 의무가 있다고 말하는것 같다.   ","categories": [],
        "tags": [],
        "url": "/2019-9-26-til/",
        "teaser": "/assets/images/HibikeQauntumSymbol_500x300_version_waifu.jpeg"
      },{
        "title": "2019년 9월 27일 TIL",
        "excerpt":"   오늘은 컨디션이 너무 안좋았다. 원래 벌떡 일어나는데 오늘은 일어날 수 없을것만 같은 기분에 알람을 15초는 듣고 있었던것 같다. 스트레스를 너무 받아서 그런가. 스트레스 때문에 금방 잠도 못들고 낮에 집중도 안되고. 스트레스를 조절할 방법을 찾아야겠다. 오늘은 차분하게 보냈다. 집에가서 잘 잘것같다. 장기전으로 가면 퍼포먼스는 멘탈과 컨디션관리에 달린것 같다.   불면증 같은거 모르고 살았는데 그때는 그냥 인생에 큰 기대를 안걸고 살아서 그랬던것 같다. 지금은 열심히 하고 싶은데.   망가지기 전에 미리 쉬어야겠다.   ","categories": [],
        "tags": [],
        "url": "/2019-9-27-til/",
        "teaser": "/assets/images/HibikeQauntumSymbol_500x300_version_waifu.jpeg"
      },{
        "title": "2019년 9월 28일 TIL",
        "excerpt":"   오늘은 커디션 굿. 스트레스만 안받으면 다음날은 괜찮은거 같다. 파이선 DB코드가 너무 더럽다. 특히나 너무 네스티드하게 짜여진 것들은 파이선 상에서 일괄처리가 너무 힘들다. 절대로 이렇게 스키마를 짜면 안되겠다. 컬렉션 갯수가 늘어나더라도 코드를 한번 알아보기 쉽고 한레벨 더 높게 짜는게 훨씬 더 생산성이 높은것 같다.   JSON버그가 났다. 자바스크립트 의 객체유형을 그대로 반영해서 JSON을 만들어주지 않아서 갑자기 데이터가 증발하는 문제였다. 자바스크립트에서 데이터를 입력하고 출력은 되긴되지만 배열이기 때문에 일반 프로퍼티였던 answer는 JSON이 되지 않은 것이다   ","categories": [],
        "tags": [],
        "url": "/2019-9-28-til/",
        "teaser": "/assets/images/HibikeQauntumSymbol_500x300_version_waifu.jpeg"
      },{
        "title": "2019년 9월 30일 TIL",
        "excerpt":"   IAM을 최대한 활용해보려다보니 비용을 조금 지불하고 있다. IAM유저들이 루트의 시크릿키를 공유해서 쓰는 줄 알고 시도하고 있었는데 알고보니 유저를 생성할때 지급받는 시크릿키가 각 개발자 입장에서 유효한거고 그걸 분실하면 유저입장에선 자발적으로 회복할 방법이 없었다.   한 동안 정신적으로 힘들었는데 지금 힘든 과정을 극복하는게 큰 자산이 될거라고 생각하니까 마음이 한결 편해졌다. 그리고 조금더 관리활동에 내 자원을 할당해야겠다고 다짐했다.   혼돈에 빠져있을때 잘 돌아오는 방법을 익혀야겠다. 똑같은 문제를 경험한 선배들의 이야기를 듣거나 읽는게 참 도움이 된다.   ","categories": [],
        "tags": [],
        "url": "/2019-9-30-til/",
        "teaser": "/assets/images/HibikeQauntumSymbol_500x300_version_waifu.jpeg"
      },{
        "title": "2019년 10월 1일 TIL",
        "excerpt":"   SPINRT3가 끝나고 프로젝트는 이제 사용할 수 있는 서비스가 되었다. Bare-minimum이지만 말이다. 처음에는 화려한 스택을 사용해서 뭔가 그럴사한걸 하고 싶었는데, 점점 서비스를 써보게끔 디테일한 예외처리나 사용자 편의적인 목표들이 점점 우선순위가 높아지고 있다. 발표할때는 볼품없을것같기도 하지만 사용자들한테 내 서비스를 보여준다고 생각하니 점점 더 디테일하게 하고 싶어진다.   어제부터 느낌이 좋다. 팀원들도 협조적이고 이제는 달려가기만 하면 된다. 컨디션 올그린   자신을 명확하게 표현하는걸 더 의식해서 연습 해야겠다. 마치 내말을 다른 사람이 꼭 들어야 한다고 자기주장을 해야한다. 지금은 사실 그렇게 말할때 내 스스로가 나를 위협적으로 느껴서 주저할때가 너무 많다.   ","categories": [],
        "tags": [],
        "url": "/2019-10-1-til/",
        "teaser": "/assets/images/HibikeQauntumSymbol_500x300_version_waifu.jpeg"
      },{
        ...
        "excerpt":"이사 이후에 CPU 쪽에서 별 부하가 없는데도 성능이 부족한것처럼 화면이 끊기는 문제가 발생하였다. 이번에 이사를 하면서 특별히 하드웨어를 탈부착한건 아니었으나 하드웨어 쪽에서 간섭이 예상되서 SATA 케이블을 다 빼서먼지를 제거해주니 정상으로 돌아왔다. 아마 이사하는 도중에 먼지가 대량으로 떨어지면서 접촉면에 붙었고 불필요한 인터럽션이 주기적으로 발생한것으로 보인다.   ","categories": [],
        "tags": [],
        "url": "/sata/",
        "teaser": "/assets/images/HibikeQauntumSymbol_500x300_version_waifu.jpeg"
      },{
        ...
        "title": "2022. 06. 13 TIL & Diary",
        "excerpt":"#### 도커, 진보신학, 경제팟캐스트, 기초화장품   Devops   도커복습 마무리. 쿠버네티스 개요 진입.   Philosophy   지금은 연재가 끝나버린 진보신학 팟캐스트를 3년째 듣고 있는데 질리지가 않는다. 이 방송의 기획자는 보수와 진보라는 양극을 오가는 학문 사조의 파도같은 변화를 잘 설명해주고 동시에 현실과 신학의 접목에 하기 위해 완벽한 답보단 애매한 고민을 강조한다. 덕분에 어디서나 쓸 수 있는 만능 프레임은 잘 주지 않는 대신 직면한 문제를 해결하기 위해선 어떤 프레임으로 해석하는게 좋은지 고민하게끔 장려한다. 그덕인지, 이게 질리지가 않는다. 어쩌면 이게 인생의 묘미. 질리지 않는 삶의 비법일지도 모르곘다.   이건 나름대로 재미가 있지만, 경제나 세상 돌아가는 이야기도 알고 싶다보니 새로운 팟캐스트를 찾는중. 오늘 이런 회사가 좋네, 누가 막말을 했네 그런 이야기보단 산업 단위의 인사이트, 다양한 자산군의 성격에 대한 소개, 역사적 흐름에 대한 나름의 추측과 근거를 대는 방송을 찾고 싶다. 내가 모든걸 할 수가 없으니 그걸 대신 연구해주고 설명해줄 수 있는 방송이 있으면 좋겠다.   Diary   안하던 짓을 다 해보고 해보고 싶다는 결의 아래 화장품도 많이 사고 있다. 비싼건 아니어도 종류나 아이템은 다양하게 사보는중(MTS 같은). 기초화장품만 따지면 어머니보다 더 많이 산것 같기도 하고. 나름 꼼꼼하게 바르고 의자에 앉아있으면 기분이 묘하게 좋단말이지.   Life      운동 시간 1시간   생체배터리70% → 30%   ","categories": [],
        "tags": [],
        "url": "/2022-06-13-til-diary/",
        "teaser": "/assets/images/HibikeQauntumSymbol_500x300_version_waifu.jpeg"
      },{
        "title": "짜잘 짜잘스 맥팁 1) 영어+한글 스펠링 체크 문제 (2) 베어 그레이브 문자 입력 불가",
        "excerpt":"짜잘 짜잘스 맥팁 1) 영어+한글 스펠링 체크 문제 (2) 베어 그레이브 문자 입력 불가   거창하게 정성스레 쓰긴 귀찮아서 메모장에 적어놓은 것들을 신속하게 업로드하는 컨셉의 #patty 태그 시리즈. 2~3개씩 내가 발견해낸것들을 올릴예정      예를 들어 “Node에’ ← 이런식으로 영어에 한글조사를 붙이면 맥의 맞춤법 검사가 틀린다고 판단하는 문제. 보통 맞춤법 검사를 끄라는 조언이 검색에 먼저 보이는데 내 입장에선 중요한건 영어니까, 맞춤법 검사를 끄긴 싫다. 그럴땐 keyboard 설정에 들어가 체크할 언어에서 한국어만 끄면 빨간줄이 사라지고 영어 스펠링 체크를 다 꺼야하는 참사도 피할 수 있다.   Bear app 을 쓰다보면 가끔 그레이브(`)가 입력 안될때가 있다. 원인 첫번째, system preference 류의 창에서 인풋박스를 화성화 시켜놓고 탭으로 이동해 베어앱에서 노트를 쓰는 경우이다. (system-keyboard-text 에서 자주 쓰는 명령어, 용어 들을 매일 단축어로 등록하다보니 자꾸 삐걱거려서 알게되었다) 원인 두번째, 첫번째가 아닌 경우 그냥 버그다. 다른 창을 활성화 시켰다가 다시 베어앱을 포커스하면 입력이 된다. 아무래도 그레이브 문자가 문서를 전체적으로 파싱하게 만들다보니 어디선가 꼬이는가 보다.   ","categories": [],
        "tags": [],
        "url": "/1-2/",
        "teaser": "/assets/images/HibikeQauntumSymbol_500x300_version_waifu.jpeg"
      },{
        "title": "2022. 06. 19 TIL & Diary",
        "excerpt":"#### Contorl Plane, Pod, Namespace   Devops   MS에서 만든 SQL CE(Compect)라는게 있던데 성능이 Sqlite보다 월등히 좋다. (지우기 같은 작업만 SQL 라이트가 미소하게 좋다) 그런데이 지식이 쓸모가 있을까?    쿠버네티스 — Control Plane, Node, API resource 개념, Imperative Declarative 작업방식, Pod, ReplicaSet, Deployment, Service( ClusterIP, NodePort, LoadBalancer, External), ConfigMap, Secret, Namespace, 자원할당 및 제한, Job, CronJob, DaemonSet, Ingress    이 학습과정에서 본격적으로 익숙하지 않은 영역에 들어섰다. 예전에 회의시간에 이름을 간간히 듣던거나 뭔가 우리팀이 가지고 있던 문제를 해결하기 위해서 다른 팀이 제공했던 리소스들이 보이는데 그때 이걸 이렇게 처리한거였구나 하고 깨닫고.. 사실은 돌아가는 플로우 전체를 알고 있어야 했는데 하루하루가 당장 내 앞에 놓인 일만 하느라 마음을 다독이느라 다른걸 흡수할 여력이 없었던 그날들이 아쉽다.   Shell   tree   Life      운동시간0.25hr   생체배터리(50% → 25%), 밖에 한번 나가볼까 했었는데 아침 5시 30분에 일어나서 할일도 없어서 공부하다가 불이 붙을때면 나가기가 싫어진다. 옛날에 하지 못한 공부에 대한 아쉬움의 망령들. 행복한 나날이다.   ","categories": [],
        "tags": [],
        "url": "/2022-06-19-til-diary/",
        "teaser": "/assets/images/HibikeQauntumSymbol_500x300_version_waifu.jpeg"
      },{
        "title": "2022. 06. 20 TIL & Diary",
        "excerpt":"#### 기분이 안좋은 날, 열매가 당장 안보여도   Diary   좋지 않은 일이 벌어지는 날이 있을 수도 있다. 그때 너무 무너지지 않고 평온하게 몰입할 수 있는 날이 오기를 믿는 수밖에. 기분이 안좋아도 꾸준히 하는건 포기하지 않음으로서 그런 자신을 사랑할 수 있다면 가장 좋을것 같다.   『콜레트는 죽기로 했다』라는 만화책을 요즘 보는데 명계의 왕을 섬기는 재봉사가 있다. 150년 동안 그의 주인은 외출을 안해서 자신이 만든 옷이 쓸모가 없어지자 굉장히 힘든 시간을 보내게 된다. 하지만 그럼에도 불구하고 재봉사인 자신이 무언가를 만들지 않으면 무슨 의미가 있을까하고 재봉을 놓지 않았다는 이야기가 나온다. 결국은 명계의 왕이 외출을 하면서 그 노력이 열매를 맺는데…    열매를 맺고 안맺고를 떠나 인생은 너무 길고 제정신을 지키기 위해선 그런 부단한 노력이 필요한것 같다. 진정으로 객관적으로 삶을 바라보고 살기엔 인간의 정신은 너무 여리다. 때로는 앞뒤가 안맞더라도 꾸준히 하는 자신을 사랑하는 수밖에 없을지도 모르겠다.   기분이 안좋은 날은 그냥 놓아주고 가치판단, 장기적은 판단은 미뤄둠으로서 슬픔에 삶의 관점이 침몰하지 않도록 해야지. 그런 믿음에서 운동도 하고 일기도 쓴다. 뭐 술먹고 그냥 비디오를 보고 소셜미디어에서 좋아요 누르면서 관심을 호소하는 것보단 나았을려나?   Life      운동 시간 1시간   생체배터리 60% -&gt; 10%   ","categories": [],
        "tags": [],
        "url": "/2022-06-20-til-diary/",
        "teaser": "/assets/images/HibikeQauntumSymbol_500x300_version_waifu.jpeg"
      },{
        "title": "2022. 06. 22 TIL & Diary",
        "excerpt":"#### Regex, NodeAffinity, Billy Bootcamp   Language   대충 흉내만 내던 정규식으로 제대로 일의 효율을 낼 수 있도록 고치기 시작하니까 너무 재밌어서 딴거 안하고 이것만 하고 있다. 가장 동기부여가 되는건 미래의 내 시간을 아껴줄거란, 인적 실수를 없애줄거란 기대감. 이제 어느정도 손에 익어서 작업 속도도 나고 한참 재미있는 시절을 보내고 있다.   Devops   쿠버네티스 파드 배치전략 중 Node Affinity를 활용한 방법   Diary   이번주 월요일을 나락을 찍고 점점 에너지를 회복해서 점점 불안감 없이 목표를 향해 돌진하고 있다. 업무시간 중에 집중도도 점점 좋아져서 스크린타임을 보면 폰을 보는 횟수도 확연히 줄어들고 좋다.   Life      운동 시간 1hr 오랜만에 빌리의 부트캠프 한방 땡겼다. 운동을 꾸준히 했음에도 여전히 힘들었다. 아무래도 웨이트중심으로 하다보니 유산소는 거의 못해서 지구력은 꽝인가 보다.   생체배터리 70% → 30%   ","categories": [],
        "tags": [],
        "url": "/2022-06-22-til-diary/",
        "teaser": "/assets/images/HibikeQauntumSymbol_500x300_version_waifu.jpeg"
      },{
